...
const biobrick = {
  ...
  "npmBionet": function(cb) {
    let brick = this;
    brick.log('installing project dependencies from npm. This may take a minute...');
    let command = "";
    command += "cd bionet-new && npm install && ";
    command += "mkdir -p static/build && mkdir -p pandadoc && ";
  ...
  "buildCssBionet": function(cb) {
    ...
    let command = "cd bionet-new && gulp build:css";
    exec(command, function(error, stdout, stderr) {
      if(stderr){
        brick.log(stderr);
      }
